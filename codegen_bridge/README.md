...
```toml
[build-dependencies]
bevy_fluent_codegen_bridge = { version = "0.1.3", features = ["build"] }
```
...
`fluent_typed_codegen = "0.1.4"` is a versioned dependency, not a sibling path.
...
ICU formatting without the retired Decimal adapter. Version 0.1.4 clarifies
storage-independent loading and the generated API documentation. Runtime and
bridge package versions are 0.1.3; this bridge release updates that dependency
minimum without changing its public API or adapter behavior.
